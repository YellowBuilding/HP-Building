        ...
        <div className="max-w-6xl mx-auto">
          ...
          <footer className="pt-6 text-xs" style={{ borderTop: '1px solid var(--rule)', color: 'var(--ink-soft)', marginTop: '4rem' }}>
            <div className="flex flex-wrap justify-between gap-4">
              <span>HP_Building · Interne sales tool · Cijfers indicatief, altijd valideren met gebouwgegevens.</span>
...
function SideBySideDiagram() {
  // Layout:
  // Left:  Gasketel  — square (x=15, y=30, w=195, h=180) → loss+warmte chevrons
  // Right: Warmtepomp — two squares stacked (x=510, y=30) → loss+warmte chevrons
  // Squares touch the chevrons (no gap), same total height (180px)
  // Loss bar: same height both sides (22px), blunt tip (8px protrusion)
  // Warmte chevron: remaining 158px, tip protrudes 20px

  const Y0 = 30;           // top of blocks
  const H = 180;           // total block height
  const LOSS_H = 22;       // loss bar height (same both sides)
  const Y_SPLIT = Y0 + LOSS_H;  // = 52, where loss ends / warmte begins
  const Y1 = Y0 + H;       // = 210, bottom of blocks
  const MID_W = Y0 + H / 2; // = 120, vertical midpoint for warmte chevron tip

  // Left group (Gasketel)
  const GAS_SQ_X = 15;
  const GAS_SQ_W = 195;
  const GAS_ARROW_X = GAS_SQ_X + GAS_SQ_W; // = 210, shared edge
  const GAS_ARROW_W = 185;
  const GAS_ARROW_END = GAS_ARROW_X + GAS_ARROW_W; // = 395
  const GAS_LOSS_TIP = GAS_ARROW_END + 8;           // blunt tip
  const GAS_WARMTE_TIP = GAS_ARROW_END + 20;        // warmte tip

  // Right group (Warmtepomp)
  const HP_SQ_X = 505;
  const HP_SQ_W = 195;
  const HP_ARROW_X = HP_SQ_X + HP_SQ_W; // = 700
  const HP_ARROW_W = 185;
  const HP_ARROW_END = HP_ARROW_X + HP_ARROW_W; // = 885
  const HP_LOSS_TIP = HP_ARROW_END + 8;
  const HP_WARMTE_TIP = HP_ARROW_END + 20;

  // Omgevingswarmte takes 2/3 of height, Groene elektriciteit 1/3
  const ENV_H = Math.round(H * (2 / 3));   // 120
  const ELEC_H = H - ENV_H;                // 60
  const HP_SPLIT_Y = Y0 + ENV_H;           // 150

  return (
    <svg viewBox="0 0 960 270" className="w-full h-auto" style={{ maxHeight: 380 }}>

      {/* ===== GASKETEL (left) ===== */}
      {/* Input square */}
      <rect x={GAS_SQ_X} y={Y0} width={GAS_SQ_W} height={H} fill="#888888" />
      <text x={GAS_SQ_X + GAS_SQ_W / 2} y={Y0 + H / 2 + 7} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="20" fontWeight="500" fill="white">Gas</text>

      {/* Loss bar — blunt tip */}
      <path d={`M ${GAS_ARROW_X} ${Y0} L ${GAS_ARROW_END} ${Y0} L ${GAS_LOSS_TIP} ${Y0 + LOSS_H / 2} L ${GAS_ARROW_END} ${Y_SPLIT} L ${GAS_ARROW_X} ${Y_SPLIT} Z`}
        fill="#E84545" />
      <text x={(GAS_ARROW_X + GAS_ARROW_END) / 2} y={Y0 + LOSS_H / 2 + 5} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="13" fontWeight="500" fill="white">Verlies</text>

      {/* Warmte chevron */}
      <path d={`M ${GAS_ARROW_X} ${Y_SPLIT} L ${GAS_ARROW_END} ${Y_SPLIT} L ${GAS_WARMTE_TIP} ${MID_W} L ${GAS_ARROW_END} ${Y1} L ${GAS_ARROW_X} ${Y1} Z`}
        fill="#2DBA5C" />
      <text x={(GAS_ARROW_X + GAS_ARROW_END) / 2} y={MID_W + 7} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="20" fontWeight="500" fill="white">Warmte</text>

      {/* ===== WARMTEPOMP (right) ===== */}
      {/* Omgevingswarmte square (top 2/3) */}
      <rect x={HP_SQ_X} y={Y0} width={HP_SQ_W} height={ENV_H} fill="#8FD8A1" />
      <text x={HP_SQ_X + HP_SQ_W / 2} y={Y0 + ENV_H / 2 - 8} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="16" fontWeight="500" fill="#0E1A14">Omgevings-</text>
      <text x={HP_SQ_X + HP_SQ_W / 2} y={Y0 + ENV_H / 2 + 12} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="16" fontWeight="500" fill="#0E1A14">warmte</text>

      {/* Groene elektriciteit square (bottom 1/3) */}
      <rect x={HP_SQ_X} y={HP_SPLIT_Y} width={HP_SQ_W} height={ELEC_H} fill="#2E8055" />
      <text x={HP_SQ_X + HP_SQ_W / 2} y={HP_SPLIT_Y + ELEC_H / 2 - 7} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="13" fontWeight="500" fill="white">Groene</text>
      <text x={HP_SQ_X + HP_SQ_W / 2} y={HP_SPLIT_Y + ELEC_H / 2 + 9} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="13" fontWeight="500" fill="white">elektriciteit</text>

      {/* Loss bar — blunt tip, same height as gasketel */}
      <path d={`M ${HP_ARROW_X} ${Y0} L ${HP_ARROW_END} ${Y0} L ${HP_LOSS_TIP} ${Y0 + LOSS_H / 2} L ${HP_ARROW_END} ${Y_SPLIT} L ${HP_ARROW_X} ${Y_SPLIT} Z`}
        fill="#E84545" />
      <text x={(HP_ARROW_X + HP_ARROW_END) / 2} y={Y0 + LOSS_H / 2 + 5} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="13" fontWeight="500" fill="white">Verlies</text>

      {/* Warmte chevron */}
      <path d={`M ${HP_ARROW_X} ${Y_SPLIT} L ${HP_ARROW_END} ${Y_SPLIT} L ${HP_WARMTE_TIP} ${MID_W} L ${HP_ARROW_END} ${Y1} L ${HP_ARROW_X} ${Y1} Z`}
        fill="#2DBA5C" />
      <text x={(HP_ARROW_X + HP_ARROW_END) / 2} y={MID_W + 7} textAnchor="middle"
        fontFamily="IBM Plex Sans, sans-serif" fontSize="20" fontWeight="500" fill="white">Warmte</text>

      {/* ===== BOTTOM LABELS ===== */}
      <text x={(GAS_SQ_X + GAS_WARMTE_TIP) / 2} y={Y1 + 28} textAnchor="middle"
        fontFamily="Bricolage Grotesque, sans-serif" fontSize="12" fontWeight="600" fill="#AAAAAA" letterSpacing="0.14em">GASKETEL</text>
      <text x={(GAS_SQ_X + GAS_WARMTE_TIP) / 2} y={Y1 + 48} textAnchor="middle"
        fontFamily="Bricolage Grotesque, sans-serif" fontSize="11" fontWeight="500" fill="#AAAAAA" letterSpacing="0.12em" opacity="0.75">85%–95% EFFICIËNTIE</text>

      <text x={(HP_SQ_X + HP_WARMTE_TIP) / 2} y={Y1 + 28} textAnchor="middle"
        fontFamily="Bricolage Grotesque, sans-serif" fontSize="12" fontWeight="600" fill="#8FD8A1" letterSpacing="0.14em">WARMTEPOMP</text>
      <text x={(HP_SQ_X + HP_WARMTE_TIP) / 2} y={Y1 + 48} textAnchor="middle"
        fontFamily="Bricolage Grotesque, sans-serif" fontSize="11" fontWeight="500" fill="#8FD8A1" letterSpacing="0.12em" opacity="0.75">280%–450% EFFICIËNTIE</text>
    </svg>
  );
}
